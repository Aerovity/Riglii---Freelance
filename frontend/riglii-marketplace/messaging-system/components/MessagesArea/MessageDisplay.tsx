...
export default function MessageDisplay({
  ...
  useEffect(() => {
    const fetchAttachment = async () => {
      ...
      const { data } = supabase.storage.from("message-attachments").getPublicUrl(message.attachment_url)
  ...
  if (isProjectDelivery) {
    ...
    const fileCount = fileCountMatch ? Number.parseInt(fileCountMatch[1]) : 0
                  ...
                  <div className="text-sm">
                    {daysRemaining > 0 ? (
                      <p className="text-amber-600 font-medium">
                        ⏰ The conversation will automatically close in {daysRemaining} day
                        {daysRemaining !== 1 ? "s" : ""}.
                      </p>
                    ) : (
                      <p className="text-red-600 font-medium">🔒 Conversation is closed</p>
                    )}
                  </div>
  ...
  return (
    <div className={`flex gap-3 mb-4 ${isOwn ? "flex-row-reverse pl-11" : "pr-11"}`}>
      <Avatar className="h-8 w-8 flex-shrink-0">
        ...
        <AvatarFallback>{getUserInitials(message.sender?.full_name || "Unknown")}</AvatarFallback>
      </Avatar>
