...
// Define HR team emails - you can modify this list or move it to environment variables
...
export async function generateAndSendFreelancerPDF(data: FreelancerData, customRecipients?: string[]) {
  try {
    ...
    // Use custom recipients if provided, otherwise use default HR team emails
    ...
    console.log("Sending to emails:", recipients.join(", "))
    ...
    try {
      ...
      // Emulate screen for better rendering
      ...
      for (const recipient of recipients) {
        try {
          ...
          const emailResult = await resend.emails.send({
            ...
            to: [recipient], // Changed back to array format as Resend expects
            ...
            attachments: [
              {
                ...
                content: Buffer.from(pdfBuffer).toString('base64'), // Convert Buffer to base64
              },
            ],
          })
          ...
          if (!emailResult.error) {
            ...
            console.log(`Email sent successfully to ${recipient}`, emailResult.data)
          } else {
            console.error(`Failed to send to ${recipient}:`, emailResult.error)
          }
      ...
      console.log(`Email sent successfully to ${successfulSends} recipients`)

      return { 
        success: true, 
        emailIds: emailResults.map(r => r.data?.id).filter(Boolean),
        message: `Profile created and email sent successfully to ${successfulSends} recipient${successfulSends > 1 ? 's' : ''}`,
...
// Function to get or update HR team emails (can be called from other parts of your app)
